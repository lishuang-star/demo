...
{
	CBlockingSocket *c = (CBlockingSocket *)lpParameter;
	CBlockingSocket ClientSocket = *c;//用局部变量保存线程传进来的地址传递的参数，防止主线程中socket被改写。
	...
	cout << "connect closed by client" << endl;
	return 0;
}
...
int main(int argc, char *argv[])
{
	...
	CBlockingSocket::Initialize();// 初始化
	ListenSocket.HintsAndResult(NULL, argv[1]);
	ListenSocket.Open();//指定套接字资源
	ListenSocket.Bind();//绑定
